...
const Text3DFlip = ({
  ...
  return (
    <ElementTag
      className={`text-3d-flip-container ${className}`}
...
const CharBox = memo(
  ({ char, textClassName = "", flipTextClassName = "", rotateDirection }) => (
    ...
    >
      <span
        className={`relative backface-hidden flex-center-char ${textClassName}`}
        style={{ transform: FRONT_FACE_TRANSFORMS[rotateDirection] }}
      >
        {char}
      </span>
      <span
        className={`absolute top-0 left-0 backface-hidden flex-center-char ${flipTextClassName}`}
